...
Q = np.array(list(map(int, file.readline().split())))

P = np.array(list(map(int, file.readline().split())))
""" мои значения:
...
dt = 0.2
dh = 0.001
V = np.array(list(map(float, file.readline().split())))
t = np.array(list(map(float, file.readline().split())))
h = np.array(list(map(float, file.readline().split())))
